...
const r2 = list2.reduce(soma, 0); // Se a função for vazia precisa por o elemento neutro 0 ou o valor desejavel //
const r3 = list1.reduce(produto, 1); // Nesse caso como era nultiplicação foi colocado o elemento neutro 1 //
...
// informada como parâmetro (Altera o Vetor)

function compararPorTamanho(s1, s2) {
    return s1.length - s2.length; // Compara 2 strings por tamanho de acordo com o criterio passado na aula
}

console.log("SORT -----------------------------");

const s1 = [...nomes].sort(); // [...] é um Spread que faz um clone do vetor
console.log(s1);
